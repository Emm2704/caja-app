...
<body>
    ...
    <div class="p-5 mb-4 text-bg-dark container-fluid">
        <div class="container">
            <h1 class="display-5 fw-bold">Editar Venta</h1>
        </div>
    </div>
    <div class="container">
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" class="form-horizontal" action="{{ route('sales.update', $sale->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select id="buyer_type" class="form-select" name="buyer_type" required>
                                <option value="" disabled>Seleccionar tipo de comprador</option>
                                <option value="persona" {{ $sale->nit ? '' : 'selected' }}>Persona</option>
                                <option value="entidad" {{ $sale->nit ? 'selected' : '' }}>Entidad</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group" id="entidad-fields" style="{{ $sale->nit ? '' : 'display: none;' }}">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="razon_social" name="razon_social" placeholder="Ingrese razón social" value="{{ $sale->razon_social }}">
                        </div>
                        <label class="control-label col-sm-2" for="nit">NIT:</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="nit" name="nit" placeholder="Ingrese NIT" value="{{ $sale->nit }}">
                        </div>
                    </div>
                    ...
                    <div class="card-body" id="products-container">
                        ...
                        <div class="product-row mb-3">
                            ...
                            <input type="number" class="form-control product-quantity mt-2" name="products[{{ $index }}][quantity]" placeholder="Cantidad" value="{{ $detail->quantity }}" required>
                        </div>
                        @endforeach
                    </div>
